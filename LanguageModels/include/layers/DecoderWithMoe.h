...
/**
 * @brief GPT Components (Decoder-Only)
 *
...
public:
     ...
     */
    DecoderWithMoe(std::size_t dim, std::size_t dFfDim, RandomEngine& rng, bool moeFlag = false, std::size_t numExperts = 4, std::size_t topK = 2)
        : dModel(dim), attn(dim, rng), norm1(dim), norm2(dim), useMoe(moeFlag)
    {
        if (useMoe) {
     ...
     * @param rope Optional rotary position embedding for attention;
     * nullptr disables it.
     */
    ...
    void forward(const Tensor<T>& x, Tensor<T>& out, RotaryEmbedding<T, RopeConfig>* rope = nullptr) {
        validation::requireColumns(x, dModel, "DecoderWithMoe input");
