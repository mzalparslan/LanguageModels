...
/**
 * @brief Demo stage: trains BasicGPT (plain decoder-only transformer) on the
 * repeating pattern 0, 1, 2, 0, 1, 2, ... and checks that generation stays
 * inside vocabulary.
 *
...
int testBasicGPT() {
    ...
    BasicGPT gpt(vocabSize, dModel, numLayers, ctxLen);
    ...
    for (int epoch = 0; epoch < 200; epoch++) {
        ...
        if (epoch % 50 == 0) {
            std::cout << "Epoch " << epoch 
                << " Loss: " << totalLoss / stepCount << std::endl;
        }
    }
    ...
    if (generated.back() == 2 || generated.back() == 1 || generated.back() == 0) {
        std::cout << "Test Passed: Generated meaningful "
            "tokens within vocab." << std::endl;
    }
    else {
